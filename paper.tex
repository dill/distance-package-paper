...
\Abstract{
Estimating the abundance and spatial distribution of animal and plant
populations is essential for conservation and management. We introduce
the \proglang{R} package \pkg{Distance} that implements distance
sampling methods to estimate abundance. We describe how users can obtain
estimates of abundance (and density) using the package as well
documenting the links it provides with other more specialized
\proglang{R} packages. We also demonstrate how \pkg{Distance} provides a
migration pathway from previous software, thereby allowing us to deliver
cutting-edge methods to the users more quickly.
}

\Plainauthor{David L Miller, Eric Rexstad, Len Thomas, Laura Marshall, Jeffrey L Laake}
\Plaintitle{Distance Sampling in R}
\Plainkeywords{distance sampling, abundance estimation, line transect, point transecs, detection function, Horvitz-Thompson, R, Distance}
...
\Address{
  ...
  16 Sumner Street, Woods Hole, MA 02543, USA and Centre for Research into
  Ecological and Environmental Modelling, The Observatory, St Andrews,
  ...
  E-mail: \href{mailto:Eric.Rexstad@st-andrews.ac.uk}{\nolinkurl{Eric.Rexstad@st-andrews.ac.uk}}\\
  
      Len Thomas\\
  University of St Andrews\\
  Centre for Research into Ecological and Environmental Modelling, The
  Observatory, St Andrews, Fife KY16 9LZ, Scotland\\
  E-mail: \href{mailto:len.thomas@st-andrews.ac.uk}{\nolinkurl{len.thomas@st-andrews.ac.uk}}\\
  URL: \url{http://lenthomas.org}\\~\\
      Laura Marshall\\
  University of St Andrews\\
  Centre for Research into Ecological and Environmental Modelling, The
  Observatory, St Andrews, Fife KY16 9LZ, Scotland\\
  E-mail: \href{mailto:lhm@st-andrews.ac.uk}{\nolinkurl{lhm@st-andrews.ac.uk}}\\
  
      Jeffrey L Laake\\
  Marine Mammal Laboratory\\
  Alaska Fisheries Science Center National Oceanic and Atmospheric
  Administration 7600 Sand Point Way N.E., Seattle, WA 98115, USA\\
...
onwards have included \proglang{R} \citep{rcore} packages as the
...
performed in \proglang{R} and many new methods are being developed, we
are encouraging the use of our \proglang{R} packages directly.
\proglang{R} provides a huge variety of functionality for data
exploration and reproducible research, much more than is possible in
Distance for Windows.

Until now those wishing to use our \proglang{R} packages for straight
forward distance sampling analyses would have had to negotiate the
package \pkg{mrds} \citep{mrds-pkg} designed for mark-recapture distance
sampling \citep{Burt:2014gu}, requiring a complex data structure to
perform analyses. \pkg{Distance} is a wrapper package around \pkg{mrds}
making it easier to get started with basic distance sampling analyses in
\proglang{R}. The most basic detection function estimation only requires
a numeric vector of distances. Here we demonstrate how to use
\pkg{Distance} to fit detection functions, perform model checking and
...
{\centering \includegraphics{paper_files/figure-latex/detectdists-1} 
...
\pkg{Distance} provides a selection of candidate functions to describe
...
etc. The \pkg{Distance} package also allows the incorporation of such
...
formatting for \pkg{Distance}; candidate detection function models are
described in terms of formulation and fitted examples in \proglang{R}.
...
the package in a broader context amongst other \proglang{R} packages
used for estimating the abundance of biological populations from
distance sampling data.

\section{Data}\label{data}

We introduce two example analyses performed in \pkg{Distance}: one line
...
before use with \pkg{Distance}, though there are a range of possible
...
\item
  ...
  ``Detection functions'') a \code{data.frame} is required. Each row of
  the \code{data.frame} contains the data on one observation. The
  \code{data.frame} must contain a column named \code{distance}
  ...
  \code{observer} or \code{seastate}). The column name \code{size} is
  ...
  include \code{object} and \code{detected}, these are not required for
...
\item
  ...
  included in the \code{data.frame} specifying: \code{Sample.Label}, the
  ID of the transect; \code{Effort}, transect effort (for lines their
  length and for points the number of times that point was visited);
  \code{Region.Label}, the stratum containing the transect (which may be
  from pre- or post-survey stratification, see ``Estimating abundance
  and variance''); \code{Area}, the area of the strata. Transects which
  were surveyed but have no observations must be included in the data
  set with \code{NA} for \code{distance} and any other covariates. We
...
detection function. In this case the column \code{distance} is replaced
by two columns \code{distbegin} and \code{distend} referring to the
...
The columns \code{distance}, \code{Area} and (in the case of line
transects) \code{Effort} have associated units (though these are not
explicitly included in a \pkg{Distance} analysis). We recommend that
...
are recorded in metres, the \code{Effort} columns should contain line
lengths also in metres and the \code{Area} column gives the stratum
...
\code{Distance} includes the most popular of these models. Here we
...
optimisation \citep[via the \pkg{Rsolnp} package;][]{rsolnp-pkg} to
...
day and more. In \pkg{Distance} we assume that these variables affect
...
A detection function can be fitted in \pkg{Distance} using the \code{ds}
...
Note that when there are no covariates in the model, \code{ds} will add
...
\code{plot} on the resulting model object. We can also call
...
\code{key=} argument to \code{ds}. For example, a hazard rate model can
be fitted as:
...
Here \code{ds} also fits the hazard-rate model then hazard-rate with a
...
\code{ds} assumes the data given to it has been collected as line
transects, but we can switch to point transects using the argument
\code{transect="point"}. We can include covariates in the scale
parameter via the \code{formula=~...} argument to \code{ds}. A
...
Note that here, unlike with the minke whale data, \code{ds} warns us
...
can use \code{summary} to give useful information about our fitted
...
distribution function or EDF). The Q-Q plots in \pkg{Distance} plot a
...
We can generate a Q-Q plot and test results using the \code{gof_ds}
...
without covariates or adjustments (setting \code{adjustment=NULL} will
force \code{ds} to fit a model with no adjustments):
...
e.g.][]{burnham2003model}. \pkg{Distance} includes a function to create
table of summary information for fitted models, making it easy to get an
overview of a large number of models. The \code{summarize_ds_models}
function takes models as input and can be especially useful when paired
with \pkg{knitr}'s \code{kable} function to create summary tables for
...
by the following call to \code{summarize_ds_models}:
...
showing how to estimate abundance and its variance in \proglang{R}.
...
included in the \code{summary} output and the table produced by
...
\code{Region.Label} and have a corresponding \code{Area} (if we are
...
As an example, the minke whale data contain two strata: \code{North} and
\code{South} relating to strata further away from and nearer to the
...
detail in \citet{Fewster:2009ku}. \pkg{Distance} implements all of the
...
\tightlist
\item
  \code{Summary statistics}: giving the areas, covered areas, effort,
...
\item
  \code{Abundance}: giving estimates, standard errors, coefficients of
  variation, lower and upper confidence intervals and finally the
  degrees of freedom for each stratum's abundance estimate.
\item
  \code{Density}: lists the same statistics as \code{Abundance} but for
  a density estimate.
\end{enumerate}
...
from the summary object. This object is a \code{list} of
\code{data.frames}, so we can use the \code{kable} function from
\pkg{knitr} to create summary tables of abundance estimates and measures
of precision, such as Table \ref{minke-abund}. We prepare the
\code{data.frame} as follows before using \code{kable}:
...
The features of \pkg{Distance} are deliberately limited to provide a
...
We noted at the start of the article that \pkg{Distance} is a
simple-to-use wrapper around the package \pkg{mrds}. Additional features
available in \pkg{mrds} include models that relax the assumption that
...
\pkg{Distance} can provide us with estimates of abundance or density for
...
(for example derived from GIS products). \pkg{Distance} interfaces with
one such package used to perform this type of analysis: \pkg{dsm}
...
distance sampling package \pkg{mads} \citep{mads-pkg}. In addition,
\pkg{mads} can also incorporate sightings with unknown species
...
distance sampling data can be reliably analysed. \pkg{DSsim} allows
...
\pkg{DSsim} \citep{DSsim-pkg} allows users to more easily identify
...
analyses to \proglang{R}. For that reason we have created the
\pkg{readdst} \citep{readdst-pkg} package to interface with projects
...
the data and create equivalent models in \proglang{R}. \pkg{readdst} can
also run these analyses and test the resulting statistics (for example,
\(\hat{N}\) or \(\hat{P_a}\)) calculated in \proglang{R} against those
calculated by Distance for Windows. We hope that \pkg{readdst} will
provide a useful transition to \proglang{R} for interested users.
\pkg{readdst} is currently available on GitHub at
...
analysis in \proglang{R}. We have covered the possible models for
detectability, model checking and selection and finally abundance and
variance estimation.

In combination with tools such as \pkg{knitr} and \pkg{rmarkdown}
\citep{rmarkdown-pkg}, the helper functions in \pkg{Distance} provide a
...
attention to \pkg{DsShiny} \citep{dsshiny-pkg}, a package that builds on
\pkg{Shiny} \citep{shiny-pkg} and \pkg{mrds} to allow users to build and
fit models in a graphical interface, with output to RMarkdown.

\proglang{R} and its extension packages provide many tools exploratory
...
pursue distance sampling in \proglang{R}. More information on distance
...
\pkg{Distance} and other \proglang{R} packages for analysis of distance
sampling data.

\section{Acknowledgements}\label{acknowledgements}

The authors would like to thank the many users of \pkg{Distance},
\pkg{mrds} and Distance for Windows who have contributed bug reports and
