...
```{r adjust-mix-comp, fig.width=10, fig.height=5, fig.cap="Top row shows possible shapes for the detection function when adjustments are included for half-normal and hazard-rate models. Bottom row shows possible shapes when 2-component mixtures of half-normal functions are used. Notation is as in main text.", echo=FALSE}
par(mfrow=c(2,4), mar=c(3.5, 3, 2, 1) + 0.1)
...
# mixture models
plotmix <- function(sig1, sig2, mix){
  
  gmix <- function(x) mix*g.hn(x, sig1) + (1-mix)*g.hn(x,sig2)

  curve(gmix, from=0, to=1, xlab="", ylab="",
        main=bquote(sigma[1] == .(sig1) ~ sigma[2] == .(sig2) ~  phi[1] == .(mix)),
        xlim=c(0,1), ylim=c(0,1), asp=1)
  title(xlab="Distance", line=2)
  title(ylab="Detection probability", line=2)
}
plotmix(0.8, 0.15, 0.3)
plotmix(0.6, 0.1,0.7)
plotmix(10, 0.2, 0.15)
plotmix(0.7, 0.05, 0.6)
...
The workhorse of detection function fitting in \pkg{Distance2} is the \code{ds} function. Here we show off the formulations for the detection function that we've seen above for both the minke whale and amakihi data.
...
We can specify the form of the detection function via the \code{model=df(...)} argument to \code{ds}. For example, fitting a hazard rate model with cosine adjustments of order 2 can be specified as:
```{r minke-hr-cos}
minke_hrcos <- ds(minke, truncation=1.5, model=df(model=~hr+cos(2)))
```

### Amakihi

By default \code{ds} assumes that the data given to it is line transect, but we can switch to points using \code{transect="points"}. Including covariates in the scale is via the `scale=~...` argument to `df`; a hazard-rate model for the amakihi that includes observer as a covariate (and truncating at 82.5m, given in @Marques:2007ey) can be specified by:
```{r amakihi-hr-obs}
amakihi_hr_obs <- ds(amakihi, truncation=82.5, transect="point",
                     model=df(model=~hr, scale=~obs))
```
...
```{r amakihi-hr-obs-mas}
amakihi_hr_obs_mas <- ds(amakihi, truncation=82.5, transect="point",
                         model=df(model=~hr, scale=~obs+mas))
```

```{r minke-hn-plot, fig.width=9, fig.height=9, fig.cap="Plot of fitted detection functions overlayed on the histograms of observed distances. Top row the minke whale data, half-normal model. Bottom row: average detection function for the amakihi data and marginal plot showing the levels of observer."}
par(mfrow=c(3,3))
plot(minke_hn, nomf=TRUE)
plot(1:10,1:10, main="",xlab="",ylab="",type="n", axes=FALSE)
plot(1:10,1:10, main="",xlab="",ylab="",type="n", axes=FALSE)
plot(amakihi_hr_obs, nomf=TRUE)
plot(1:10,1:10, main="",xlab="",ylab="",type="n", axes=FALSE)
plot(amakihi_hr_obs_mas, nomf=TRUE)
```
