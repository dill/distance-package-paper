...
For many years distance sampling analyses have been available via the Windows program Distance (or "DISTANCE"); for clarity henceforth "Distance for Windows"). From version 5 of Distance for Windows, R packages have been included to perform particular analyses [@Thomas:2010cf, further details of the history of Distance for Windows are included in that paper]. This paper shows how to fit detection functions, perform model checking and selection and estimate abundance in \proglang{R} using the package \pkg{Distance}. \pkg{Distance} is a wrapper package around the more complex (and more powerful) \pkg{mrds} and offers a subset of the analyses possible with that package.
...
It is also possible to use distances collected in intervals ("binned", as opposed to "exact" distance data) to model the detection function. In this case a set of cutpoints for the bins must be provided rather than the \code{distance} column, these are specified as two columns \code{distbegin} and \code{distend}.
