...
The Windows program Distance (or "DISTANCE"; for clarity henceforth "Distance for Windows") has been available for many years to perform distance sampling analyses (and before that program "TRANSECT"). From version 5 of Distance for Windows, \proglang{R} packages have been included to perform some parts of an analysis [@Thomas:2010cf, further details of the history of Distance for Windows are included in that article]. This article shows how to fit detection functions, perform model checking and selection, and estimate abundance in \proglang{R} [@rcore] using the package \pkg{Distance} [@distance-pkg]. \pkg{Distance} is a wrapper package around the more complex (and more powerful) \pkg{mrds} [@mrds-pkg] and offers a subset of the analyses possible with that package.
