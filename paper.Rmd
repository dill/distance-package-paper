...
abstract: >
  (LT: I have gone through putting comments to myself about things I need to come back and edit.  In the end, I have written them within brackets in the text because I couldn't get the todo package to work, because square brackets are interpreted as markdown.  In general, we might have been better to have used .Rnw but och well.  Anyway DLM or others, feel free to comment on my comments!  (Sorry if comments seem a bit negative -- I'd rather have had time to do the editing on this round so you can see my suggested text -- but it always takes me ages to do this bit and I had limited time right now -- I'll come back to it!...))
  (LT: Need to define abundance; need to introduce concept of density also.) The estimation of the abundance of a biological population is an essential part of applied ecology and wildlife management. (LT: This sentence not strictly accurate, probably needs some editing.) To ensure that abundance estimates are accurate, we must account for uncertain detectability of plants and animals; distance sampling is a popular method for doing this. Here we show how models for detectabilty can be fitted using the \proglang{R} package \pkg{Distance}. (LT: Is it actually helpful to use the jargon like Horvitz-Thompson-like in the abstract?  Almost no-one will know what we mean.) We also show how \pkg{Distance} can be used to calculate Horvitz-Thompson-like estimates of abundance, its interface to other packages to provide sophisticated analyses and a migration pathway from previous software.
keywords:
...
(LT: I wonder who the intended audience is for this paper?  The way it is currently written, it ``kinda'' introduces distance sampling as well as the relevant commands.  Is this standard for a paper in this journal -- to introduce the methods as well as the software?  Can we assume a reasonable level of analytical sophistication on the part of the reader, and so start with a few more formulae?  Can we assume they know the methods, more or less, or point them at somewher where this is already introduced.  (Perhaps I should go back and have a look at the EcolApps paper to see what approach we took there...))

(LT: Need to define abundance, and add in the concept of density as well as population size.) Distance sampling [@Buckland:2001vm; @Buckland:2004ts; @buckland2015distance] encompasses a suite of field methods and statistical models used to estimate the abundance of biological populations. (LT: I would think we might want to put a little more here, defining plot sampling, fo rexample.) Distance sampling field procedure can be thought of as an extension of plot sampling, where we wish to take into account the decreasing probability of detecting objects at increasing distance from the sampler. (LT: Detection function is used without defining it.) We do this by building a model for detectability and use quantities calculated from the detection function to adjust the observed counts to obtain an estimate of abundance.

(LT: Possibly edit to add the actual years.) The Windows program Distance (or "DISTANCE"; for clarity henceforth "Distance for Windows") has been available for many years to perform distance sampling analyses (and before that program "TRANSECT"). From version 5 of Distance for Windows, \proglang{R} packages have been included to perform some parts of an analysis [further details of the history of Distance for Windows are included in @Thomas:2010cf]. (LT: This won't mean much unless we've defined detection function first -- possibly need to do a little more work introducing the method and being explicit about where to go to find out more -- e.g., encyclopaedia articles, free online stuff, etc.  The concepts mostly are explained a little further down here, so possibly just some re-ordering needed.) Here we show how to fit detection functions, perform model checking and selection, and estimate abundance in \proglang{R} [@rcore] using the package \pkg{Distance} [@distance-pkg]. \pkg{Distance} is a wrapper package around the more complex (and more powerful) \pkg{mrds} package [@mrds-pkg] and offers a subset of the analyses possible with that package.

## Distance sampling

(LT: a census is a complete count; I don't really think that sample surveys fall under this.  So, a bit of editing needed here.) Census-type surveys (quadrat or strip transects assuming perfect detectability within strips or quadrats) are inefficient (requiring considerable field effort) and we should expect that not all objects (animals, plants, dung, etc) can be observed. Accounting for imperfect detectability is an important consideration when obtaining accurate estimates of abundance [@LahozMonfort:2013bg]. Using the extra information gained by recording distance from the sampler to the observation, it is possible to model detectability. Because we expect detectability to decrease with increasing distance from the sampler, we model detectability as a function of distance (plus perhaps other covariates, see below). (LT: The word detection function is used again without it having been defined explicitly.) We use the probabilities of detection calculated from the detection function to estimate abundance in the area covered by the survey, which we then scale-up to the study region of interest.

Distance sampling surveys are conducted using two transect types: line and point transects. In line transect sampling observers walk (or fly, sail, etc)  lines, observing objects and recording their perpendicular distance to the line; whereas in point transect sampling observers remain stationary at a location and record distances to objects from that point. Field methods are chosen to be suitable to species and habitat constraints [@buckland2015distance].

```{r points-and-lines, fig.height=6, fig.width=7, echo=FALSE, message=FALSE, fig.cap="Left side plots show an example of a survey of an area containing a population of 500 objects; blue lines (top plot) and crosses (bottom plot) indicate sampler placement, red dots indicate detected individuals and grey dots give the locations of unobserved individuals. The right side of the figure shows histograms of observed distances (again, lines top and points bottom).\\label{fig:pointslines}"}
...
(LT: stranded one-sentence paragraph -- needs integrating.) Figure \ref{fig:pointslines} shows sampling of a population of 500 individuals using line and point transects and their corresponding histograms.
...
We show examples of analyses in \pkg{Distance} using two data sets: one line transect and one point transect. (LT: I think it might be better to delete this next sentence -- after all, two datasets can't possibly represent all the possibilities there are in the real world -- CDS, MCDS, MRDS, with and without covariates, problems such as spiked data, animal movement, measurement error, etc etc.) These data sets have been chosen to be representative of data seen in practice.
