...
Model uncertainty can be addressed using standard maximum likelihood theory. We can invert the Hessian matrix of the likelihood to obtain a variance-covariance matrix. We can then pre- and post-multiple this by the derivatives of $\hat{N}_\text{C}$ 
$$
...
The \code{varn} manual page gives further advice and technical detail on this matter.

## Estimating abundance and variance in R

Going back to the minke whale data, we have the information we require to calculate $A$ and $a$ above, so we can estimate abundance and its variance. When we supply data to \code{ds} in the ``flat file'' format given above, \code{ds} will automatically calculate abundance estimates based on the survey setup in the data.
