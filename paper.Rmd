...
DLM: if you look at the unmarked paper they do a fair bit of explanation of the methods.-->
...
plot(x, y, xlab = "Perpendicular distance", ylab = "Probability of detection", type = "l", ylim = c(0,1.1), xaxp = c(0,1,5), cex = set.cex, cex.lab = set.cex.lab)
...
plot(x, y, xlab = "Radial distance", ylab = "Probability of detection", type = "l", ylim = c(0,1.1), xaxp = c(0,1,5), cex = set.cex, cex.lab = set.cex.lab)
...
```
Here \code{ds} had also fitted a model with cosine adjustments (the default) but finds the AIC improvement to be insufficient to select the adjustment. 
...
Though fitting the detection function is the primary modelling step in distance sampling, we are really interested in estimating detectability, and from that density or abundance. It is also important to calculate our uncertainty for these estimates. This section addresses these issues mathematically before showing how to estimate abundance and its variance in \proglang{R}.
...
The authors would like to thank the many users of \pkg{Distance}, \pkg{mrds} and Distance for Windows who have contributed bug reports and suggestions for improvements over the years. We would particularly like to thank Steve Buckland, David Borchers, Tiago Marques, Jon Bishop and Lorenzo Milazzo for their contributions.
