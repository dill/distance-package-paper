...
Figure \ref{fig:pointslines} shows simulated sampling of a population of 500 objects using line and point transects and their corresponding histograms of observed detection distances. Note that for the purposes of distance sampling an object may either refer to an individual in a population or a cluster of individuals. In the latter instance, the density or abundance of clusters is estimated separately to the mean cluster size which can later be used to estimate the density or abundance of individuals. 
...
* *Non-increasing*: we do not think that observers should be more likely to see distant objects than those nearer the transect. If this occurs, it usually indicates an issue with survey design or field procedures (for example that the distribution of objects with respect to the line, $\pi(y)$ is not what we expect), so we do not want the detection function to model this. _(LM: Hmm could be avilability but could also be detectability... either way it is bad news but don't want to give people the impression they should just chop these bumps off without thought do we? Ideally spot early in data collection and resolve)_
...
There are many factors that can affect the probability of detecting an object. These include things like observer skill, cluster/group size (if objects occur in groups), the vessel or platform used, sea state, other weather conditions and time of day. In \pkg{Distance} we assume that these variables affect detection only via the scale of the detection function (and do not affect the shape).
...
Fitting a detection function in \pkg{Distance} is done using the \code{ds} function. Here we show some of the possible formulations for the detection function we have seen above applied to the minke whale and amakihi data.
...
```
Note that \code{ds} will automatically select adjustment terms using minimum AIC and show the selection steps.
...
Note that here, unlike with the minke whale data, \code{ds} warns us that we have only supplied enough information to estimate the detection function (not density or abundance).
 ...
 _(LM: this next model was thrown in without much explanation, I have tried to add some intro to it... is this correct?)_
 
While automatic AIC selection is performed on adjustment terms, model selection for covarariates must be performed manually. Here we introduce a second covariate model with both observer and minutes after sunrise explaining detectability. We will compare these models further in the following section.
...
par(mfrow=c(1, 3))
plot(minke_hn, showpoints=FALSE, main = "Minke~distance", cex = 2)
plot(amakihi_hr_obs, pdf=TRUE, cex=0.7, main = "Amakihi~distance+observer")
plot(amakihi_hr_obs_mas, pdf=TRUE, cex=0.7, main = "Amakihi~distance+observer+minutes")
```
...
To judge how well a detection functions fits the data when we have exact distances, we use a quantile-quantile plot (Q-Q plot). Here the Q-Q plot compares the the cumulative distribution function (CDF) of the fitted detection function to the distribution of the data (empirical distribution function or EDF). The Q-Q plots in \pkg{Distance} plot a point for every observation. The EDF is is the proportion of points that have been observed at a distance equal to or less than the distance of that observation. The CFD is calculated from the fitted detection function as the probability of observing an object at a distance less than or equal to that of the given observation. This can be interpreted as assessing whether the number of observations up to a given distance is in line with what the model says they should be. As usual for Q-Q plots, "good" models will have values close to the line $y=x$, poor models will show greater deviations from that line.
...
_(LM: hmm we say that the hn model should be discarded based on fit but it is then included in the table... is this confusing? There is also no guidance here on model selection... although the AIC is smallest for amakihi.hr.obs.mas, some would argue that a difference of less than 2 does not really justify the inclusion of the additional covariate. Are we simply saying minimum AIC wins (if so this is not clear)? Should we point them at further discussion on the topic?)_
...
<!-- LM The location of this Table needs changing it appears in the middle of the output. -->
...
We noted at the start of the article that \pkg{Distance} is a simple-to-use wrapper around the package \pkg{mrds}. Additional features available in \pkg{mrds} include  models that relax the assumption that detection is certain at zero distance from the transect. This is done using mark-recapture type methods which require a different survey technique, known as double observer surveys [see @Burt:2014gu for an introduction].
...
In combination with tools such as \pkg{knitr} and \pkg{rmarkdown} [@rmarkdown-pkg], the helper functions in \pkg{Distance} provide a useful set of tools to perform reproducible analyses of wildlife abundance for both managers and ecologists. In addition, users should remember the other basic functions in \proglang{R} such as \code{hist} that enable straight forward exploratory data analysis. We hope that this paper provides useful examples for those wishing to pursue distance sampling in \proglang{R}. More information on distance sampling can be found at \url{http://distancesampling.org} and a mailing list is maintained at \url{https://groups.google.com/forum/#!forum/distance-sampling}.
