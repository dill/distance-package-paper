...
In conventional distance sampling the observer is either stood at a point or moving along a line and we only assume they see everything that occurs exactly at that point or on the line. It is then expected that the further an object is from the sampler (the point or line) the less likely it is that the observer will see it. By recording the distance to each of the detected objects we can fit a model to describe the probability of detection given distance from the sampler, which we refer to as the ‘detection function’.  The detection function can be used to infer how many objects were missed and thereby produce estimates of density and/or abundance. (LM: perhaps move this? Further information on distance sampling, including its application, various extensions to deal with more complex situations and recent methodological developments can be found at http://distancesampling.org/dbib.html.)

The Windows program Distance (or "DISTANCE"; for clarity henceforth "Distance for Windows") was first released in November 1998 as Distance for Windows version 3.5 and was initially based on its predecessor “TRANSECT”. Since this time it has evolved to include various design and analysis features. Distance for Windows versions 5 onwards have included \proglang{R} packages as the analysis engines providing additional more complex analysis options than those offered by the original FROTRAN engines. (LM to finish: The current version of Distance links to 4 R packages... Development in R progressing rapidly so allowing users to access new tools faster?)

(LM Add part here to say what this paper describes… pathway to sustainability using R?) Further details of the history of Distance for Windows are included in @Thomas:2010cf. 
