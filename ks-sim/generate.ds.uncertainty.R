...
generate.ds.uncertainty <- function(ds.object){
  ...
  while(dud.df){
    ...
    }else{# if(ds.object$ds$aux$ddfobj$scale$formula=="~1"){
...
vars<-all.vars(as.formula(ds.object$ds$aux$ddfobj$scale$formula))
  xmat[, vars] <- ds.object$ds$aux$ddfobj$xmat[, vars]
}
...
if(ds.object$ds$aux$ddfobj$scale$formula!="~1"){
  vars<-all.vars(as.formula(ds.object$ds$aux$ddfobj$scale$formula))
  dists[, vars] <- ds.object$ds$aux$ddfobj$xmat[, vars]
}
