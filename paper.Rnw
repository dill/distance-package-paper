...
\begin{figure}
\begin{center}
<<label=detectdists, fig=TRUE, echo=FALSE, message=FALSE, height=5.5, width=9>>=
...
\begin{figure}
\begin{center}
<<label=points-and-lines, fig=TRUE, echo=FALSE, message=FALSE, warning=FALSE, width=7, height=6>>=
...
\begin{figure}
\begin{center}
<<label=hn-hr-par-comp, fig=TRUE, echo=FALSE, height=5, width=10>>=
...
\begin{figure}
\begin{center}
<<label=adjust-mix-comp, fig=TRUE, echo=FALSE, height=3, width=10>>=
...
\begin{figure}
\begin{center}
<<label=minke-amakihi-hn-plot, echo=FALSE, warning=FALSE, fig=TRUE, height=4, width=9>>=
par(mfrow=c(1, 3))
...
\begin{figure}
\begin{center}
<<label=amakihiqq, fig=TRUE, echo=FALSE, message=FALSE, height=3, width=6>>=
par(mfrow=c(1,2))
...
<<label=minke-abundance-table, results=tex, echo=FALSE>>=
minke_table <- summary(minke_hn)$dht$individuals$N
minke_table$lcl <- minke_table$ucl <- minke_table$df <- NULL
...
Results \pkg{Rdistance} models can be use in combination with \proglang{R} modelling functions such as \code{lm}, \code{glm} etc to build abundance estimates which vary according to covariates. More information is available on the project's wiki.
